...
async def try_connect_ws(
    ...
    url = f"{WS_BASE}{path}"
    try:
        async with websockets.connect(url, ping_interval=None, timeout=timeout_seconds) as ws:
            try:
        ...
        return (False, code, str(e).encode())
...
async def terminal_session(
    ...
    try:
        async with websockets.connect(url, ping_interval=None, max_size=2**20, timeout=timeout_seconds) as ws:
